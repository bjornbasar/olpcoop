...
class Shanty_Mongo_Connection_Group
{
	...
	public function addConnections ($connectionOptions)
	{
		...
		if (array_key_exists('slave', $connectionOptions))
			$slaves[] = $connectionOptions['slave']; // single slave
	...
	public function formatConnectionString (array $connectionOptions = array())
	{
		...
		if (array_key_exists('hosts', $connectionOptions))
			$hosts = $connectionOptions['hosts'];
		else
	...
	public function formatHostString (array $hostOptions = array())
	{
		...
		if (isset($hostOptions['host']) && ! is_null($hostOptions['host']))
			...
			// Set port
		$hostString .= ':';
		if (isset($hostOptions['port']) && ! is_null($hostOptions['port']))
			$hostString .= $hostOptions['port'];
		else
			$hostString .= '27017';
